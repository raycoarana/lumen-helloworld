 ...
 *   (E.g. this only includes direct composer dependencies - You may have to manually add indirect composer
 *   dependencies to 'directory_list')
...
return [
    ...
    // (E.g. `@return the|null` will still look for a class with the name `the`, but `@return the` will be ignored
    ...
    // even if those return types aren't available in reflection (real types were taken from php 7.3 or 8.0-dev,
    // depending on target_php_version).
    //
    // Note that with php 7 and earlier, php would return null or false for many internal functions if the argument
    // types or counts were incorrect.
    // As a result, enabling this setting with target_php_version 8.0 may result in false positives for
    // `--redundant-condition-detection` when codebases also support php 7.x.
    ...
    // [here](https://github.com/phan/phan/tree/v4/.phan/plugins).
    //
